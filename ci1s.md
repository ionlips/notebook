...
-   **plin3-r160**: 171,658 atoms with 14 replicas requiring 500 ns of
    ...
    2026.3. Replica exchange will be carried out every 1,000 steps.
...
-   ~~GROMACS version 2026.0 patched with the aforementioned pull request
    branch (this may or may not work)~~.
...
> [!IMPORTANT]
> Replica exchange via `-hrex` is not supported in any PLUMED-patched GROMACS
> version after 2024.3. The only install procedure to follow is [PLUMED version
> 2.9.4-patched GROMACS version 2024.3].
...
a PLUMED patch. Thus, we choose GROMACS version 2024.3 for this patch. As per
the [code-specific notes], we should ensure that the following CMake options
are disabled/enabled: `-DGMX_MPI=ON` and `-DGMX_THREAD_MPI=OFF`.
...
curl \
    ...
    -L \
    -o /project/home/p201412/.local/src/$PKG/$VER.tgz
tar \
    --one-top-level=$VER \
    --strip-components=1 \
    -C /project/home/p201412/.local/src/$PKG \
...
```shell
PKG=plumed VER=2.10
git clone https://github.com/plumed/plumed2 /project/home/p201412/.local/src/$PKG/$PKG
cd /project/home/p201412/.local/src/$PKG/$PKG
git switch v$VER
git fetch origin pr/1439/head:pr-1439
git swtich pr-1439 -- patches gromacs-2026.0.diff
./configure \
    --prefix=/project/home/p201412/.local/opt/$PKG-$VER \
    --enable-rpath \
    LDFLAGS="-L$EBROOTFLEXIBLAS/lib -Wl,-rpath,$EBROOTFLEXIBLAS/lib" \
...
> [!NOTE]
> See this [GROMACS merge request] - I may need to build GROMACS with this fix
> applied in order for the PLUMED patch to work. Currently, we install and
> patch GROMACS without this fix.

```shell
PKG=gromacs_plumed-2.10 VER=2026.0
mkdir -p /project/home/p201412/.local/src/$PKG
curl https://ftp.gromacs.org/gromacs/gromacs-2026.0.tar.gz -L -o /project/home/p201412/.local/src/$PKG/$VER.tar.gz
tar \
    --one-top-level=$VER \
    --strip-components=1 \
    -C /project/home/p201412/.local/src/$PKG \
...
```shell
PKG=gromacs_nvshmem-3.4.5 VER=2026.3
mkdir -p /project/home/p201412/.local/src/$PKG
...
```shell
PKG=gromacs_cufftmp-25.3 VER=2026.3
mkdir -p /project/home/p201412/.local/src/$PKG
curl https://ftp.gromacs.org/gromacs/gromacs-2026.3.tar.gz -L -o /project/home/p201412/.local/src/$PKG/$VER.tar.gz
tar \
    --one-top-level=$VER \
...
cmake \
    ...
    -DCMAKE_CUDA_ARCHITECTURES=80 \
    -DCMAKE_INSTALL_PREFIX=/project/home/p201412/.local/opt/$PKG-$VER \
    -DCMAKE_INSTALL_RPATH=$NVHPC/Linux_x86_64/25.3/comm_libs/12.8/nvshmem/lib \
...
[PLUMED version 2.9.4-patched GROMACS version 2024.3]: ci1s.md#plumed-version-2.9.4-patched-gromacs-version-2024.3
...
[GROMACS merge request]: <https://gitlab.com/gromacs/gromacs/-/merge_requests/6133>
